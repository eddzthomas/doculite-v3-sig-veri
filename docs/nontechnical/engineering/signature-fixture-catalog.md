...
The automated replay tests check the structure of the samples and the recorded service behavior (for example, that the multi-signature sample really carries two signature containers, and how the document service reports an upload it cannot process). That level of per-signature trust evidence was the M0-D verifier work: the replay tests still check structure only, while the adapter itself now produces per-signature results.
